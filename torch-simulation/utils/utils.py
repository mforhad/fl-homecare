...
def test(model, test_loader, device, y_test=None, groups_test=None):
    criterion = torch.nn.CrossEntropyLoss()
...
def read_secret_keys():
    with open("utils/secrets/accesskey.txt", "r") as f:
        access_key = f.read()
    
    with open("utils/secrets/secretkey.txt", "r") as f:
        secret_key = f.read()
